...
const uniPush = uniCloud.getPushManager({appId:"__UNI__9265F9E"}) //注意这里需要传入你的应用appId，用于指定接收消息的客户端
exports.main = async (event, context) => {
	log(event)
	const allParams = JSON.parse(event.body)
	log("入参",allParams)
	//0 消息推送 1 investment 2 event 3 purchase
	return await uniPush.sendMessage({
		...
		"payload": {
			...
			purchaseId:allParams.purchaseId || "",
			fundId:allParams.fundId || "",
		}
	})
};