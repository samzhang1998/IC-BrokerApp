  ...
  <BasePage :title="`${currentEmploymentItem?.statusCode} - ${currentEmploymentItem?.typeStatus}`" hasBack>
    <wd-form :model="formData" ref="formRef" class="flex-col gap-4">
      ...
      <FormItem label="Position Title">
        <wd-input type="text" v-model="formData.positionTitle" placeholder="Enter position title" />
      </FormItem>
      <FormItem label="Occupation">
        <wd-input type="text" v-model="formData.occupation" placeholder="Enter position title" />
      </FormItem>
      <FormItem label="Employer Type">
        <wd-picker :columns="employerColumns" v-model="formData.employerType" placeholder="Select employer type" />
      </FormItem>
      ...
      <FormItem label="On Probation">
        <wd-switch v-model="formData.onProbation" size="medium" />
      </FormItem>
      <FormItem label="Fully Maintained Company Car">
        <wd-switch v-model="formData.maintainedCompanyCar" size="medium" />
      </FormItem>
      ...
      <FormItem v-for="item in incomeJson" :key="item.name" :label="item.name">
        <view class="flex-y-center gap-20rpx">
          ...
          <wd-picker :columns="incomeTimeColumns" v-model="item.time" placeholder="Select income time" class="flex-1" />
        </view>
      </FormItem>
    </wd-form>
